...
def get_grid_size():
    ...
    while True:
        ...
        prompt = prompt_font.render("Enter Grid Size N:", True, (255, 255, 255))
        window.blit(prompt, (WINDOW_WIDTH // 2 - prompt.get_width() // 2, 150))

        user_text = prompt_font.render(input_value, True, (0, 255, 0))
        window.blit(user_text, (WINDOW_WIDTH // 2 - user_text.get_width() // 2, 250))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if input_value.isdigit() and int(input_value) >= 5:
...
class SnakeGame:
    ...
    def display_game_over(self):
        ...
        while True:
            ...
            play_again_text = font.render("Play Again", True, play_again_color)
    ...
    def game_loop(self):
        ...
        while True:
            ...
            window.blit(score_text, [self.width // 2 - score_text.get_width() // 2, 
                                   TASKBAR_HEIGHT // 2 - score_text.get_height() // 2])

            pygame.display.update()
            clock.tick(self.snake_speed)

if __name__ == "__main__":
    temp_window = pygame.display.set_mode((600, 400))  # temporary window for input
    window = temp_window
